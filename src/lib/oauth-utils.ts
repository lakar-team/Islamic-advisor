...
/**
 * Centrally manages the OAuth login redirection.
 * Now initiates from the BACKEND to ensure server-side persistence of PKCE verifier.
 */
export async function initiateLogin(returnTo: string = 'landing'): Promise<void> {
  try {
    ...
    if (!response.ok) throw new Error(data.error || 'Failed to initiate login');

    // The backend returns the full authorization URL
